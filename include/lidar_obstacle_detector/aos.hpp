...
namespace travel {
    ...
    class ObjectCluster {
        ...
        public:
            ObjectCluster(ros::NodeHandle *nh) : node_handle_(*nh) {
                empty_ = {};
                // Init ROS related
                ROS_INFO("Inititalizing ObjectCluster...");

                node_handle_.param<float> ("/lidar/min_range"     , MIN_RANGE, 0.0);
                node_handle_.param<float> ("/lidar/max_range"     , MAX_RANGE, 30.0);
                node_handle_.param<int>   ("/lidar/vert_scan"     , VERT_SCAN, 64);
                node_handle_.param<int>   ("/lidar/horz_scan"     , HORZ_SCAN, 1800);
                node_handle_.param<float> ("/lidar/min_vert_angle", MIN_VERT_ANGLE, -30.0);
                node_handle_.param<float> ("/lidar/max_vert_angle", MAX_VERT_ANGLE, 50.0);

                node_handle_.param<int>  ("/aos/downsample"           , DOWNSAMPLE, 2);
                node_handle_.param<float>("/aos/th_horz_merg"         , HORZ_MERGE_THRES, 0.3);
                node_handle_.param<float>("/aos/th_vert_merg"         , VERT_MERGE_THRES, 1.0);
                node_handle_.param<int>  ("/aos/vert_scan_size"       , VERT_SCAN_SIZE, 4);
                node_handle_.param<int>  ("/aos/horz_scan_size"       , HORZ_SCAN_SIZE, 4);
                node_handle_.param<int>  ("/aos/horz_skip_size"       , HORZ_SKIP_SIZE, 4);
                node_handle_.param<int>  ("/aos/horz_extension_size"  , HORZ_EXTENSION_SIZE, 3);

                node_handle_.param<int>  ("/aos/min_cluster_size", MIN_CLUSTER_SIZE, 4);
                node_handle_.param<int>  ("/aos/max_cluster_size", MAX_CLUSTER_SIZE, 100);
                ...
                ROS_INFO("Max vertical angle : %f", MAX_VERT_ANGLE);
                ROS_INFO("Min vertical angle : %f", MIN_VERT_ANGLE);
                ROS_INFO("Vertical resolution: %f", resolution);
                ROS_INFO("max_cluster_size: %d", MAX_CLUSTER_SIZE);
                ROS_INFO("min_cluster_size: %d", MIN_CLUSTER_SIZE);

            }

            ~ObjectCluster() {}

            boost::shared_ptr<pcl::PointCloud<T>> segmentObjects(boost::shared_ptr<pcl::PointCloud<T>> cloud_in) {
                // 0. reset
                max_label_ = 1;
                clusters_.clear();
                ...
                start = chrono::steady_clock::now();
                boost::shared_ptr<pcl::PointCloud<T>> cloud_out = boost::make_shared<pcl::PointCloud<T>>();
                ...
                for (size_t i = 0; i < valid_indices.size(); i++) {
                    ...
                    for (auto &p : clusters_[idx]) {
                        if (p->valid) {
                            ...
                            cloud_out->points.emplace_back(cloud_in->points[p->idx]);
                            pt_cnt++;
                        } else {
