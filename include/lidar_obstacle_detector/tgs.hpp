...
namespace travel {
    ...
    public:
        TravelGroundSeg(ros::NodeHandle *nh) : node_handle_(*nh) {
            ...
            ROS_INFO("Inititalizing TravelGroundSeg...");

            node_handle_.param<double> ("/lidar/min_range"     , MIN_RANGE_, 0.0);
            node_handle_.param<double> ("/lidar/max_range"     , MAX_RANGE_, 30.0);
          
            node_handle_.param<double>("/tgs/resolution"   , TGF_RESOLUTION_, 5.0);
            node_handle_.param<int>  ("/tgs/num_iter"     , NUM_ITER_, 3);
            node_handle_.param<int>  ("/tgs/num_lpr"      , NUM_LRP_, 20);
            node_handle_.param<int>  ("/tgs/num_min_pts"  , NUM_MIN_POINTS_, 10);
            node_handle_.param<double>("/tgs/th_seeds"     , TH_SEEDS_, 0.4);
            node_handle_.param<double>("/tgs/th_dist"      , TH_DIST_, 0.3);
            node_handle_.param<double>("/tgs/th_outlier"   , TH_OUTLIER_, 0.3);
            node_handle_.param<double>("/tgs/th_normal"    , TH_NORMAL_, 0.707);
            node_handle_.param<double>("/tgs/th_weight"    , TH_WEIGHT_, 1.5);
            node_handle_.param<double>("/tgs/th_obstacle"  , TH_OBSTACLE_HEIGHT_, 1.5);
            node_handle_.param<double>("/tgs/th_lcc_normal", TH_LCC_NORMAL_SIMILARITY_ , 1.5);
            node_handle_.param<double>("/tgs/th_lcc_planar", TH_LCC_PLANAR_MODEL_DIST_ , 1.5);
            node_handle_.param<bool> ("/tgs/refine_mode"  , REFINE_MODE_, true);
            node_handle_.param<bool> ("/tgs/visualization", VIZ_MDOE_, true);
            ...
            ROS_INFO("num iter: %d", NUM_ITER_);
            ROS_INFO("num lpr: %d", NUM_LRP_);
            ROS_INFO("num min points: %d", NUM_MIN_POINTS_);
            ROS_INFO("th seeds: %f", TH_SEEDS_);
            ROS_INFO("th dist: %f", TH_DIST_);
            ROS_INFO("th outlier: %f", TH_OUTLIER_);
            ROS_INFO("th normal: %f", TH_NORMAL_);
            ROS_INFO("th weight: %f", TH_WEIGHT_);
            ROS_INFO("th obstacle: %f", TH_OBSTACLE_HEIGHT_);
            ROS_INFO("th lcc normal: %f", TH_LCC_NORMAL_SIMILARITY_);
            ROS_INFO("th lcc planar: %f", TH_LCC_PLANAR_MODEL_DIST_);
        };

        // Converted inputs to Eigen::Vector3d so that we can feed Open3D vectors in
        std::pair<pcl::PointCloud<PointType>, pcl::PointCloud<PointType>> estimateGround(const pcl::PointCloud<PointType>& cloud_in){            
            ...
            cloudGround_out.width = cloudGround_out.points.size();
            cloudGround_out.height = 1;
            cloudGround_out.is_dense = true;
            cloudNonground_out = ptCloud_tgfwise_nonground_;
            cloudNonground_out.width = cloudNonground_out.points.size();
            cloudNonground_out.height = 1;
            cloudNonground_out.is_dense = true; 
            std::pair<pcl::PointCloud<PointType>, pcl::PointCloud<PointType>> segmented_clouds (cloudNonground_out, cloudGround_out);           
